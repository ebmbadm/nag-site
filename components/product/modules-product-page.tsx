...
import { Chip, Container, Eyebrow, Surface } from "@/components/ds";
...
/** Glue a number to its unit so a value never breaks as "20 / кГц" on a phone. */
function keepUnits(value: string) {
  return value.replace(/(\d)\s(?=[A-Za-zА-Яа-я%°])/g, "$1\u00a0");
}

/**
 * Datasheet plate for the parameters shared by every module in the series.
 * Leader dots run label → value the way the printed manual sets them; the
 * protection list is too long for a leader row, so it breaks out into chips.
 */
function SpecPlate({ rows }: { rows: SpecRow[] }) {
  if (rows.length === 0) return null;

  const protection = rows.find((row) => row.label === "Защита");
  const leaderRows = rows.filter((row) => row !== protection);

  return (
    <div className="mt-12 border border-border bg-surface lg:mt-[54px]">
      <header className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1 border-b border-border px-6 pb-4 pt-5">
        <Eyebrow accent>Общие характеристики</Eyebrow>
        <span className="font-mono text-2xs uppercase tracking-[var(--ls-label)] text-text-faint">
          Едины для TDS-10 · TDH-10 · TDS-20 · TDH-20
        </span>
      </header>

      <dl className="px-6 py-4 lg:columns-2 lg:gap-x-14">
        {leaderRows.map((row) => (
          <div
            key={row.label}
            className="flex break-inside-avoid items-baseline gap-3 py-3"
          >
            <dt className="shrink-0 text-sm text-text-muted">{row.label}</dt>
            <span
              aria-hidden
              className="h-px min-w-8 flex-1 text-border-strong"
              style={{
                backgroundImage:
                  "repeating-linear-gradient(90deg, currentColor 0 1px, transparent 1px 5px)",
              }}
            />
            <dd className="text-right font-mono text-sm tabular text-text">{keepUnits(row.value)}</dd>
          </div>
        ))}
      </dl>

      {protection ? (
        <dl className="border-t border-border px-6 py-5">
          <dt className="font-mono text-2xs uppercase tracking-[var(--ls-label)] text-text-faint">
            {protection.label}
          </dt>
          <dd className="mt-3 flex flex-wrap gap-2">
            {protection.value.split(",").map((item) => (
              <Chip key={item}>{item.trim()}</Chip>
            ))}
          </dd>
        </dl>
      ) : null}
  ...
  return (
    <Surface mode="dark">
      <section className="border-b border-border py-12 lg:py-[68px]">
        <Container className="enter-stagger grid items-center gap-9 lg:grid-cols-2">
          <div>
      ...
      <section className="border-b border-border">
        <Container className="reveal-stagger grid gap-0 lg:grid-cols-2">
          <article className="border-x border-border p-6 lg:min-h-[390px]">
            {subwoofer ? (
              <div className="bg-photo-backdrop">
                <Image
                  src={subwoofer.src}
                  alt={subwoofer.alt}
                  width={1600}
                  height={1200}
                  sizes="(min-width: 1024px) 50vw, 100vw"
                  className="h-[240px] w-full object-contain p-3 mix-blend-multiply"
                />
              </div>
            ) : null}
      ...
      {product.models && product.models.length > 0 ? (
        <section className="pb-[66px]">
          <Container className="reveal-fade-stagger grid gap-px bg-border sm:grid-cols-2 lg:grid-cols-4">
      ...
      {product.specMatrix ? (
        <section className="pb-20">
          <Container className="reveal-stagger">
            <div className="grid items-start gap-10 lg:grid-cols-[0.75fr_1.25fr] lg:gap-[54px]">
              <aside>
              ...
              {/* min-w-0: the comparison table has a 560px floor and would otherwise
                  stretch the grid column past the viewport on phones. */}
            ...
            <SpecPlate rows={product.specGroups[0]?.rows ?? []} />
          </Container>
        </section>
      ) : null}
    </Surface>
