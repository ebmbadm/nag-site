...
describe("oKompanii", () => {
  test("links to the complete published history through 2019", () => {
    ...
    expect(oKompanii.cards[0].kicker).toBe("1976–2019 · история NOVIK");
    expect(oKompanii.cards[0].text.toLowerCase()).toContain("единая история");
    expect(oKompanii.cards[0].href).toBe("/istoriya");
